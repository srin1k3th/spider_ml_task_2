...
| Module | Purpose |
|--------|---------|
| `ingest.py` | Loads MedQuAD CSV + guideline markdown files, chunks text, saves processed JSON |
| `build_index.py` | Embeds chunks with all-MiniLM-L6-v2, builds FAISS IndexFlatIP, saves index + metadata |
...
**Why FAISS IndexFlatIP?** Corpus is <100k chunks — exact cosine search is fast enough, no need for approximate methods. 

**Why two-stage retrieval?** Bi-encoder (all-MiniLM-L6-v2) is fast but shallow. Cross-encoder (ms-marco) is slow but accurate. FAISS narrows 100k→20, cross-encoder picks the best 5, ensuring best of both worlds.
...
**Misspelling handling**: Embedding similarity naturally handles typos ("diabeties" is still close to "diabetes" in vector space). No explicit spell-correction needed.
...
- **Retrieval quality**: Compare FAISS top-20 vs reranked top-5 — does reranking surface more relevant chunks?
...
- **Confidence calibration**: Do HIGH confidence answers actually have better evidence than LOW ones?
